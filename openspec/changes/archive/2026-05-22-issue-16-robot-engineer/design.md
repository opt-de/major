...
为自动化专业新增"机器人工程师"岗位内容包，遵循现有 `content/majors/automation/` 下的数据结构和 schema 约束。所有数据以 JSON 文件存储，通过 `npm run validate-content` 校验。

## Goals / Non-Goals

**Goals:**
- 在 `content/majors/automation/jobs/` 下新增 `robot-engineer.json` 岗位定义
- 在 `content/majors/automation/abilities/` 下新增 4 个能力节点 JSON
- 在 `content/majors/automation/tasks/` 下新增 1 个学习任务 JSON
- 在 `content/majors/automation/resources/` 下新增 5 个资源建议 JSON
...
| 决策 | 选择 | 理由 |
|------|------|------|
| 岗位归属 | 自动化专业 (automation) | 机器人工程是自动化领域的核心方向，避免新建专业增加 scope |
| 数据结构 | 遵循现有 template JSON | 保持与第一期示例一致，通过 schema 校验 |
| 数据来源 | type: "mock" | 第一期全部使用模拟数据，后续可替换为真实来源 |
| 能力级别 | basic/intermediate/advanced 混合 | 覆盖从入门传感器集成到高级路径规划的完整梯度 |
| 文件命名 | 英文 kebab-case | 与现有 embedded-software-engineer 保持一致 |
| 资源类型 | book/tool/course/project | 覆盖机器人学习的典型资源形态 |
