...
### Requirement: Robot engineer job definition
Content SHALL define the "机器人工程师" job role under the "automation" major, including job description, work scenarios, typical tasks, and ability references.

#### Scenario: Job file exists
- **WHEN** content is loaded
- **THEN** `robot-engineer.json` SHALL exist in `content/majors/automation/jobs/`
...
- **THEN** `major.json`'s `relatedJobs` SHALL include `"robot-engineer"`
...
The job SHALL define at least 4 typical work scenarios for robot engineers.

#### Scenario: Work scenarios present
- **WHEN** the job file is inspected
- **THEN** `workScenarios` SHALL contain at least 4 entries describing real robot engineering contexts
