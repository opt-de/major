...
```bash
node ./.agents/skills/openspec-buddy/scripts/select-next-change.mjs < selection-input.json
```

For the common case, use the wrapper:

```bash
./.agents/skills/openspec-buddy/scripts/select-next-change.sh [current-series]
```
