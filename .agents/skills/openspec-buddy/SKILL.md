...
```bash
./.agents/skills/openspec-buddy/scripts/check-config.sh
```
...
6. Validate the prepared body before creating or updating the issue:
   ```bash
   ./.agents/skills/openspec-buddy/scripts/parse-issue-metadata.mjs <issue-body-file>
   ```
7. Add labels:
   - `status:ready`
   - `area:<area>`
...
9. If this is a planned series, create or identify the series parent issue, then link the child issue:
   ```bash
   ./.agents/skills/openspec-buddy/scripts/create-series-parent.sh <series>
   ./.agents/skills/openspec-buddy/scripts/link-issue-parent.sh <parent-issue> <child-issue>
   ```
10. If this issue depends on another change issue, link the native relationship:
   ```bash
   ./.agents/skills/openspec-buddy/scripts/link-issue-dependencies.sh <blocked-issue> <blocking-issue>
   ```
11. Add the created issue to the default GitHub Project:
   ```bash
   ./.agents/skills/openspec-buddy/scripts/add-issue-to-project.sh <issue-url>
   ```
...
5. Claim the issue with a remote branch lock:
   ```bash
   ./.agents/skills/openspec-buddy/scripts/claim-change.sh <issue-number>
   ```
...
8. After entering the claim branch, mark the issue in progress:
   ```bash
   ./.agents/skills/openspec-buddy/scripts/mark-in-progress.sh <issue-number>
   ```
...
11. After opening the ready PR, configure PR metadata before review:
   ```bash
   ./.agents/skills/openspec-buddy/scripts/configure-pr-metadata.sh <issue-number> <pr-url>
   ```
...
12. Mark the issue in review:
   ```bash
   ./.agents/skills/openspec-buddy/scripts/mark-review.sh <issue-number> <pr-url>
   ```
...
6. Mark the issue archived:
   ```bash
   ./.agents/skills/openspec-buddy/scripts/mark-achieved.sh <issue-number> <archive-path> [pr-url]
   ```
